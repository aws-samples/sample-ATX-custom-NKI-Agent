"""AgentCore entry point — NKI kernel optimization agent (Strands SDK 1.x scaffold).
...
Ported from the pre-1.0 `agentcore/app.py` into the current `agentcore create`
(v0.11.0) project layout (`app/<name>/main.py` package). The hot path uses the
raw Bedrock `converse` / `invoke_model` boto3 APIs directly (stable across the
SDK jump); the Strands `Agent`/`tool` objects remain registered for parity with
the scaffold's tool-registration model. The `@app.entrypoint` handler returns a
plain dict synchronously — bedrock-agentcore serializes that as the JSON
response body, which is exactly what `scripts/eval_nkibench_agentcore.py`'s
`json.loads(body)` consumes.
"""
...
Hard rules:
- Import the NKI API with EXACTLY these names — the module is `neuronxcc.nki`, NOT a
  bare `nki`: `import neuronxcc.nki as nki`, `import neuronxcc.nki.language as nl`,
  and `import neuronxcc.nki.isa as nisa` if needed. A bare `import nki` / `import
  nki.language as nl` does NOT exist on the deployed Neuron SDK and every call then
  fails with "API 'nki.language...' is not supported" — always use the neuronxcc prefix.
